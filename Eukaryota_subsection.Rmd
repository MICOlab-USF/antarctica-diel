...
#Cleaning up data by removing blank spaces at the beginning of entries in the Domain column.
tax.func$Domain <- ifelse(substring(tax.func$Domain,1,1) == " ", substring(tax.func$Domain,2), tax.func$Domain)

#Filtering out all the rows with Bacteria data, leaving only Eukaryote data
...
Eukaryota$SequenceID <- unlist(lapply(Eukaryota$SequenceID,function(x){unlist(strsplit(x,split = ".",fixed = TRUE))[1]}))
...
This code chunk is the second version, and the previous code-chunk before this one is almost identical, the difference is that this code-chunk creates a final file that contains TPM data, and the other chunk creates a file containing the numread data.
...
# Eukaryota.noRep$Class <- ifelse(substring(Eukaryota.noRep$Class,nchar(Eukaryota.noRep$Class)-7) == " (Class)",
#                              substring(Eukaryota.noRep$Class,1,nchar(Eukaryota.noRep$Class)-8),
#                              Eukaryota.noRep$Class) #Is this statement even needed?
# 
...
for(i in 1:nrow(dfOut2)){
  if(i == 1){
    vec <- rep(NA,nrow(dfOut2))
  }
  
  vec[i] <- (1:length(order.vec))[order.vec == dfOut2$ToD[i]]
  
  if(i == nrow(dfOut2)){
    dfOut2 <- cbind(dfOut2,vec)
    names(dfOut2)[names(dfOut2) == "vec"] <- "ToD.number"
  }
}
...
# name_vec <- COG_categories$Short_name
name_vec <- COG_categories$Name_and_Category
...
filter <- dfOut2$COG.vec == "Other"
dfOut2 <- dfOut2[!filter,]
...
#              scales = "free_y",
#              labeller = function(variable,value){return(name_vec[value])})+
#   labs(y = expression(paste("Transcripts per million",sep = "",)),
...
#                                     size = 8.2),
#         legend.position = "none")+
#   guides(linetype = guide_legend(title = "Day"),
#          color = "none")
# 
# p7
...
file_name_vec <- paste("COG_Class_plots/","COG_plot2.png",sep = "")

ggsave(file_name_vec,width = 8,height = 8,dpi = 300)
```
...
top5.vec <- names(table(df.merged2$Class)[order(-table(df.merged2$Class))[1:6]])
...
order.vec <- c("Cryptophyceae", "Prymnesiophyceae", "Dinophyceae", "Bacillariophyta", "Dictyochophyceae", "Other", "Unclassified" ,"Pyramimonadales")

order.idx <-c("A","B","C","D","E","F","G","H")
...
for(i in 1:length(dfOut2$Class.vec)){
  if(i == 1){
    vec <- rep(NA,length(dfOut2$Class.vec))
    vec2 <- rep(NA,nrow(dfOut2))
  }
  
  vec[i] <- order.idx[order.vec == dfOut2$Class.vec[i]]
  vec2[i] <- (1:length(ToD.vec))[ToD.vec == dfOut2$ToD[i]]
  
  if(i == length(dfOut2$Class.vec)){
    dfOut2 <- cbind(dfOut2,vec,vec2)
    names(dfOut2)[names(dfOut2) == "vec2"] <- "ToD.number"
  }
}

names(order.vec) <- order.idx

# p8 <- ggplot(data = dfOut2)+
#   geom_line(aes(x = log(par,2),
#                 y = Count,
#                 linetype = Day.num))+
#   geom_point(aes(x = log(par,2),
#                 y = Count,))+
#   geom_errorbar(aes(x = log(par,2),
#                     y = Count,
#                     ymax = Count+sd,
#                     ymin = Count-sd))+
#   facet_wrap(facets = "vec",
#              nrow = 2,
#              ncol = 4,
#              #scales = "free_y",
#              labeller = function(variable,value){return(order.vec[value])})+
...
file_name_vec <- paste("COG_Class_plots/","Class_plot2_fixed.png",sep = "")

ggsave(file_name_vec,width = 8,height = 5,dpi = 300)
```
...
# df.merged <- separate(df.merged,col = KEGG_ko,into = c("KEGG1","KEGG2","KEGG3","KEGG4","KEGG5",
#                                                        "KEGG6","KEGG7","KEGG8","KEGG9","KEGG10",
#                                                        "KEGG11","KEGG12","KEGG13","KEGG14","KEGG15",
#                                                        "KEGG16","KEGG17","KEGG18","KEGG19"),
#                       sep = ",", remove = FALSE, fill = "right", extra = "drop")

df.merged <- separate(df.merged,col = PFAMs,into = c("PFAMs1","PFAMs2","PFAMs3","PFAMs4","PFAMs5",
                                                       "PFAMs6","PFAMs7","PFAMs8","PFAMs9","PFAMs10",
                                                       "PFAMs11","PFAMs12","PFAMs13","PFAMs14","PFAMs15",
                                                       "PFAMs16","PFAMs17","PFAMs18","PFAMs19"),
                      sep = ",", remove = FALSE, fill = "right", extra = "drop")

test_df <- df.merged[1:100,]
