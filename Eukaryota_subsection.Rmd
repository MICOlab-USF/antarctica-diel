...
p3 <- ggplot(dfOut2)+
  ...
  scale_x_continuous(breaks = unique(meta.data$datetime),
                     labels = unique(meta.data$AssemblyGroup))+
  scale_y_continuous(label = function(y){y*100})+
  labs(title = "Relative numreads per day",
       y = "numreads per class / total numreads per day (%)",
       x = "Time of sample")+
  theme_classic()+
...
ggsave("Class_light_plot.png",width = 10,height = 6,dpi = 300)
```
...
p4 <- ggplot(dfOut2)+
  ...
  labs(title = "Relative numread of each COG category for full data-set",
       y = expression(paste("Relative numread (10"^-3,")",sep = "",)),
...
ggsave("COG_light_plot.png",width = 15,height = 9,dpi = 300)
```

```{r}
df.merged <- Eukaryota.merged

Class.plot <- "Dinophyceae"
filter1 <- df.merged$Class == Class.plot
...
p6 <- ggplot(dfOut2)+
  ...
  facet_wrap(facets = "COG.vec",
             ...
             labeller = function(variable,value){return(name_vec[value])})+
  scale_y_continuous(label = function(y){y*100})+
  labs(title = paste("Relative numread of each COG category for ",Class.plot,sep = ""),
       y = expression(paste("Relative numread (%)",sep = "",)),
       x = "Day and time of sampling")+
  theme_classic()+
  theme(axis.text.x = element_text(angle = 90,
                                   vjust = 0.5,
...
save.name <- paste(Class.plot,"COG_light_plot.png",sep = "")
