...
COG.vec <- unique(df.merged$COG_category)
n <- length(COG.vec)
...
for(i in 1:n){ #Looping through the Classes to count the occurrences.
  ...
  COG.filter <- df.merged$COG_category == COG.vec[i]
...
df.merged2 <- df.merged[df.merged$COG_category != "-",]

top5.vec <- names(table(df.merged2$COG_category)[order(-table(df.merged2$COG_category))[1:11]])
...
ggsave("COG_light_plot.png",width = 10,height = 6,dpi = 300)
```
