...
```{r load_data}
tpm <- read.csv("salmon.merged.TPM.subset5.csv") #transcripts per million
names(tpm)[1] <- "SequenceID" #rename sequence ID column to match annotation files

counts <- read.csv("salmon.merged.numreads.subset5.csv")
names(counts)[1] <- "SequenceID"

funtax <- read.csv("TaxonomicAndFunctionalAnnotations.csv", sep = "\t") #functional AND taxonomic annotation file 
...
fun <- read.csv("full_function.csv", sep = "\t") #functional annotation only file  - has some extra information about functional annotation

meta <- read.csv("dielRNA_eukrythmic.tsv", sep = "\t")
...
```{r}
funtax_bact_keggplot_min1 <- funtax_bact_keggplot %>% select(ko, SampleID, counts, SampleName) %>% group_by(ko, SampleID, SampleName) %>% mutate(kosum = sum(counts)) %>% dplyr::slice(1) %>% ungroup()

funtax_bact_keggplot_min1$ko[funtax_bact_keggplot_min1$kosum < 0.5] <- "z< 0.5% abund."

koplot<- funtax_bact_keggplot_min1 %>% ggplot(aes(fill=ko, y=kosum, x=SampleName)) + 
    geom_bar(position="stack", stat="identity") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + scale_fill_manual(values = colors30)
