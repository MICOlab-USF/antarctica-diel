...
df.0filtered <- df.count[apply(df.count, 1, function(x){sum(x == 0)}) < 7,]
...
for(j in 1:4){
  ...
  df_meta$testGroup <- unlist(lapply(df_meta$ToD,function(x){ifelse(x == testGroup,testGroup,"other")}))
  ...
  mtext(testGroup, side = 3, line = -1.5, outer = TRUE)
  ...
  with(res,
       plot(log2FoldChange,
            -log10(pvalue),
            pch=20,
            main=paste0(testGroup," Volcano plot"),
            ylim=c(0,y_range),
            xlim=c(-x_range,x_range)))
  ...
  if(dim(dfOut.temp)[1] != 0){
   dfOut.temp$testGroup <- testGroup 
...
for(i in 1:4){
  ...
  for(j in 1:nrow(df_UpDown)){
   ...
   #               ifelse(df.temp$log2FoldChange[df.temp$ko == rownames(df_UpDown)[j]]<0,0,2),
   #               1)
