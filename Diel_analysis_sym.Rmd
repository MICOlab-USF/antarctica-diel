...
#          "Day2_night")
# 
# dfOut2$Day_int <- unlist(lapply(dfOut2$Day,function(x){(1:8)[x == vec]}))
# dfOut2$Day_num <- unlist(lapply(dfOut2$Day,
#                                  function(x){unlist(strsplit(x,
#                                                              split = "_",
#                                                              fixed = TRUE))[1]}))
# 
# dfOut2 <- dfOut2[dfOut2$kegg.vec %in% top5.vec[1:5],]
# 
# p1 <- ggplot(dfOut2)+
#   geom_point(aes(color = kegg.vec,
#     # color = factor(kegg.vec,
#     #                          levels = c("Unclassified","Other",top5.vec)),
#                y = Count,
#                x = Day_int))+
#   geom_line(aes(color = kegg.vec,
#     # color = factor(kegg.vec,
#     #                          levels = c("Unclassified","Other",top5.vec)),
#                 linetype = Day_num,
#                y = Count,
...
```{r}
load("CSV_files/Full.merged.RData")
source("pathway_API.R")

met_module <-  paste("ko:",pathway_API("M00017")$definition,sep = "")
Cobal_module <-  paste("ko:",pathway_API("M00122")$definition,sep = "")

idx.vec <- unique(unlist(lapply(c(Cobal_module,met_module),function(x){unlist(grep(x,Full.merged$KEGG_ko))})))

df.merged <- Full.merged[idx.vec,]

kegg.vec <- c(met_module,Cobal_module)
...
for(i in 1:n){ #Looping through the Classes to count the occurrences.
  ...
  kegg.filter <- grepl(kegg.vec[i],df.merged$KEGG_ko)
  df.temp <- df.merged[kegg.filter,]
  ...
  for(j in 1:m){ #Looping through the sampling time points.
   ...
   if(i == 1 & j == 1){ #Sets up an empty data.frame to input the data
     ...
     df.count <- cbind.data.frame(df.count,kegg.vec)
   }
...
dfOut2$Col_group <- NA
dfOut2$Col_group[dfOut2$kegg.vec %in% met_module] <- "Met"
dfOut2$Col_group[dfOut2$kegg.vec %in% Cobal_module] <- "Cobal"
...
purple <- c("#E7D7CE","#A699A9","#434582","#81347D", "#B5218E","darkblue")

colors30 <- c(blue, purple, red, yellow, orange, green, "black")

met_col <- c(red,yellow,green)
cobal_col <- c("blue","red","green","yellow","grey","black")

names(met_col) <- met_module
names(cobal_col) <- Cobal_module

ggplot(dfOut2)+
  geom_point(aes(x = factor(Day,levels = order.vec),
                 y = Count,
                 colour = kegg.vec,
                 group = kegg.vec),
             size = 3)+
  geom_line(aes(x = factor(Day,levels = order.vec),
                 y = Count,
                 colour = kegg.vec,
                 group = kegg.vec),
            linewidth = 1)+
  facet_wrap(Col_group~.,
             scale = "free_y")+
  scale_colour_manual(values = c(met_col,cobal_col))+
  theme_classic()
```
...
load("CSV_files/PairWise_DE.RData",verbose = T)
