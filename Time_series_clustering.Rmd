...
for(i in 1:length(phase.peaks)){
  ...
  dfOut2$ColorGroup[filter] <- unlist(lapply(df.temp$Sig.rat,function(x){ifelse(x > df.temp$Sig.rat[df.temp$COG == "TotPhase"],
                                                                              "Higher","Lower")}))
...
for(i in 1:length(phase.peaks)){
  ...
  for(j in 1:length(KEGG_paths)){
    ...
    df.sig[i,j] <- ifelse(vec1+vec2 == 0,0,ifelse(vec1+vec2 == 1,0,vec1/(vec1+vec2)))-phase.rat
...
# dfOut$TotPhase <- apply(dfOut[,KEGG_paths],1,sum)
# 
# dfOut2 <- dfOut %>% pivot_longer(cols = all_of(c(KEGG_paths,"TotPhase")),
#                                  names_to = "Paths",
#                                  values_to = "Count")
# 
# dfOut2$TotPhase.KEGG <- rep(dfOut2$Count[dfOut2$Sig == "Insig"] + dfOut2$Count[dfOut2$Sig == "Sig"],2)
# 
# 
# dfOut2 <- dfOut2[dfOut2$Sig == "Sig",]
# dfOut2$Sig.rat <- dfOut2$Count/dfOut2$TotPhase.KEGG
# 
# dfOut2$Sig.rat[dfOut2$TotPhase.KEGG == 0] <- 0

pheatmap(dfOut[1:12,KEGG_paths])
```
