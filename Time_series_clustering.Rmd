...
dfOut <- t(apply(vec[,df_meta$SampleID],1,function(x){x - lm(unlist(x)~df_meta$time_points)$fitted.values}))
...
vec <- salmon.TPM[sample(nrow(salmon.TPM),size = 300000,replace = F),]
names(vec) <- unlist(lapply(names(vec),
                                 function(x){unlist(strsplit(x,
                                                             split = "_",
                                                             fixed = TRUE))[1]}))
rownames(vec) <- vec$Name
vec <- vec[,-1]
...
dfOut <- merge(dfOut,rainresult,by = "Gene")
# dfOut$Group <- paste(dfOut$phase,dfOut$peak.shape,sep = "_")

dfOut <- pivot_longer(dfOut,cols = 2:25,names_to = "ToD",values_to = "TPM")
# dfOut$ToD <- unlist(lapply(dfOut$ToD,function(x){(1:8)[Order_vec == x]}))
dfOut$ToD <- unlist(lapply(dfOut$ToD,function(x){df_meta$time_points[df_meta$SampleID == x]}))

df.summ <- summarySE(dfOut,measurevar = "TPM",groupvars = c("Gene","ToD"))

dfOut <- pivot_wider(df.summ,id_cols = "Gene",
                      names_from = "ToD",
                      values_from = "TPM")
