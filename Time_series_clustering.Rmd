...
for(i in 1:length(phase.peaks)){
  if(i == 1){
    df.Fisher <- data.frame(matrix(NA,nrow = length(phase.peaks),ncol = length(COG_categories)))
    
    names(df.Fisher) <- COG_categories
...
for(i in 1:length(phase.peaks)){
  if(i == 1){
    df.Fisher <- data.frame(matrix(NA,nrow = length(phase.peaks),ncol = length(KEGG_paths)))
    
    names(df.Fisher) <- KEGG_paths
    row.names(df.Fisher) <- phase.peaks
  }
