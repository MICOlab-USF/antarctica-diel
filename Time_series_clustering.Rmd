...
for(i in 1:length(phase.peaks)){
  ...
  dfOut2$ColorGroup[filter] <- unlist(lapply(df.temp$Sig.rat,function(x){ifelse(x > df.temp$Sig.rat[df.temp$COG == "na"],
                                                                              "Higher","Lower")}))
...
# ggplot(dfOut2)+
#   geom_point(aes(x = factor(COG,levels = c("na",COG_categories[COG_categories != "na"])),
#                  y = Sig.rat,
#                  color = ColorGroup),
#              size = 2)+
