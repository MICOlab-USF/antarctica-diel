...
vec_rows <- order(apply(df.count,1,mean),decreasing = TRUE)[1:20]
# vec_cols <- order(apply(df.count2,2,mean),decreasing = TRUE)
...
for(i in 1:n){
  ...
  for(j in 1:m){
    ...
    df.count[i,j] <- log(sum(df.Temp[,names(df.Temp) == sample_vec[j]]))
  }
}
        ...
        for(l in 1:length(ko.vec)){
          ...
          if(l == length(ko.vec)){
            ...
            if(length(ko.vec) < 5){
              ...
              top5_ko <- ko.vec[order(vec2,decreasing = TRUE)[1:5]]
            }
...
names(df_genes) <- c("ko","Pathway","Class","Identifier","Path")
...
Class_df <- data.frame(df_genes[,c(2,3)])
names(Class_df) <- c("Pathway","Class")
...
```{r}
Class_of_interest <- c("Bacillariophyta","Dinophyceae","Prymnesiophyceae","Cryptophyceae","Dictyochophyceae","Unclassified")
df.merged <- Euk.Sub
df.merged$Class[is.na(df.merged$Class)] <- "Unclassified"

df.merged <- df.merged[df.merged$Class %in% Class_of_interest,]
...
Class_cols <- c('#f0b08f','#60555f','#9fd6e6','#63a375',"orange","black")
names(Class_cols) <- Class_of_interest
...
# name_vec <- paste0("Heatmaps/","Top6Class_Top10Paths.pdf")
# 
# pdf(name_vec, height = 10, width = 8)
# heat_plot
# dev.off()
...
name_vec <- paste0("Heatmaps/","Bact_all_keggs.pdf")
...
for(i in 1:n){
  ...
  for(j in 1:m){
    ...
    df.count[i,j] <- ifelse(vec != 0,log(vec),-7.25)
...
paths_df <- rbind(Euk_KO_paths[Euk_KO_paths$K1 %in% kegg.vec,],c("-",""))
