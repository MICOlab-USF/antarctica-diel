...
for(i in 1:n){
  ...
  for(j in 1:m){
    ...
    df.count[i,j] <- ifelse(vec != 0, log(vec),-5.2)
  }
}
...
df.merged <- Bact.Sub[Bact.Sub$Class %in% Order_of_interest,]
# df.merged$KEGG1[is.na(df.merged$KEGG1)] <- "Unclassified"
...
for(i in 1:length(Order_of_interest)){
  ...
  filter <- df.merged$Class %in% Order_of_interest[i]
...
for(i in 1:n){
  ...
  for(j in 1:m){
    ...
    df.count[i,j] <- ifelse(vec != 0, log(vec),-8)
    # df.count[i,j] <- sum(df.Temp[,names(df.Temp) == sample_vec[j]])
  }
}
